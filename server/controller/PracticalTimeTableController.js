import { PracticalExamTimetable, Regulation, Semester, Course, Subject, Student } from '../models/index.js';

// Get all timetables
export const getTimetables = async (req, res) => {
  try {
    const rows = await PracticalExamTimetable.find().sort({ examDate: 1, startTime: 1 });
...
export const getTimetableById = async (req, res) => {
  try {
    const row = await PracticalExamTimetable.findById(req.params.id);
...
export const createTimetable = async (req, res) => {
  try {
    const b = req.body;
    const row = await PracticalExamTimetable.create({
...
export const updateTimetable = async (req, res) => {
  try {
    ...
    const row = await PracticalExamTimetable.findByIdAndUpdate(req.params.id, update, { new: true });
    res.json({ success: true, data: row });
  } catch (err) {
    res.status(500).json({ success: false, message: "Failed" });
  }
...
export const deleteTimetable = async (req, res) => {
  try {
    await PracticalExamTimetable.findByIdAndDelete(req.params.id);
...
export const getMasterSubjects = async (req, res) => {
  try {
    const q = req.query;
    const filter = { subType: 'P' }; // Practical
